...
export const isDark = (theme: MantineTheme) => {
...
export const mergeSx = (
  ...
  if (sx instanceof Array) {
    ...
    sx.forEach((s) => (merged = { ...merged, ...s }));
    return merged;
  }

  return sx instanceof Function ? sx(theme) : sx;
};
...
export const getStyles = <T extends string>(
  ...
  styles?: Styles<T, Record<string, any>>
) => {
  return styles instanceof Function ? styles(theme, params, context) : styles;
};
